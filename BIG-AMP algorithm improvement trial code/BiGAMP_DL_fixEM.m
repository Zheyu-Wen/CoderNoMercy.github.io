function [estFin,optFin,EMoptFin,estHist]...
    = BiGAMP_DL_fixEM(Y,problem,BiGAMPopt, PrioriIn, EMopt)
...
% remove EM by jwzhang Aug  ? which one .m file has the EM algorithm?
...
defaultOptions.learn_mean = true; %learn the mean of the active X entries
...
%Iteration control
defaultOptions.maxEMiter = 10; %maximum number of EM cycles
defaultOptions.tmax = 10; %first EM iteration to use full expression for noise variance update

%Tolerances
BiGAMPopt.tol = 1e-20;
defaultOptions.EMtol = BiGAMPopt.tol; %convergence tolerance
defaultOptions.maxTol = 1e-20;%largest allowed tolerance for a single EM iteration
...
if ~isfield(EMopt,'noise_var') || isempty(EMopt.noise_var)
    if ~BiGAMPopt.sparseMode
        EMopt.noise_var = PrioriIn.noiseVar;
    else
...
if ~isfield(EMopt,'lambda') || isempty(EMopt.lambda)
    lambda = PrioriIn.lambda * ones(N,L); %**********
else
    lambda = EMopt.lambda;
end
...
if isnumeric(EMopt.init)
    ...
    if strcmp(EMopt.init,'data') %init from the data
        ...
        while(whichCol <= N) && (drawAttempts < 20)
            ...
            %Try to draw the initial dictionary %% what's mean ?
...
BiGAMPopt.Avar0 = 1*ones(M,N);
BiGAMPopt.xvar0 = 1*nuX.*ones(N,L);
...
while stop < 2
    ...
    %stop = stop + 1;
    %Prior on A  %% ? how to get this function ?
    gA = AwgnEstimIn(0, 1);
...
%     end
    ...
    if ~isempty(state)
        state.xhat(state.xhat<1e-3)=0;
        state.xhatBar(state.xhatBar<1e-3)=0;
        state.xhatBarOpt(state.xhatBarOpt<1e-3)=0;
        state.xhatOpt(state.xhatOpt<1e-3)=0;
    end
    %Run BiG-AMP
   
    [estFin2,BiGAMPopt,estHist2,state] = BiGAMP_KL_method(gX, gA, gOut, problem, BiGAMPopt,state, PrioriIn,stop); % state evolution test 
    ...
    disp(['It ' num2str(t,'%04d')...
        ...
        ' Avg Spar = ' num2str(sum(sum(p1))/L,'%3.3f')...
...
%     errZ = estHist2.errZ;
%     save('D:/AMP_Neural/_result/Big_AMP_errZ_DL_change_size1.mat','errZ');
end
errZ = estHist2.errZ;
save('D:/AMP_Neural/_result/Big_AMP_errZ_DL_change_size1.mat','errZ');
%% Cleanup
