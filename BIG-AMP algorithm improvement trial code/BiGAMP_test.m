function [estFin, optFin, estHist, state] = ...
    BiGAMP_test(gX, gA, gOut, problem, opt, state, PrioriIn)
%% state evolution test function
...
% Get options
if (nargin < 5)
    opt = BiGAMPOpt();  %%% ? BiGAPOpt function
elseif (isempty(opt))
    opt = BiGAMPOpt();
end
...
%If in sparse mode, ensure that sparseMult2 has been mexed  what's the
%function of sparsemult2?
if sparseMode
...
if sparseMode
    ...
    sMult = @(arg1,arg2) sparseMult2(arg1.',arg2,rLoc,cLoc);%%%&&&&&&&&&&&&&&&&&&&What's the sparseMult2?
...
if maskFlag
    ...
    %Create mask matrix if not in sparse mode %%隐蔽矩阵
    if ~sparseMode
...
if isempty(state) %if no state is provided
    ...
    if ~isempty(opt.shat0)
     ...
     %Make sure that size is correct if in sparse mode,numel 返回矩阵数组个数
        if sparseMode
...
while ~stop
    ...
    if ~uniformVariance
        ...
        if ~sparseMode
            ...
            zvar = sMult(Avar,xhat2) + sMult(Ahat2,xvar);  %%%? what's mean of the sMult function?
        end
    ...
    if (saveHist)
        ...
        if diagnostics
            ...
            if Ahat==zeros(M,N)
                ...
                pMatrix = find_permutation(A, Ahat);
            end
            tempAhat = Ahat*pMatrix;
            tempxhat = pinv(pMatrix)*xhat;
            
            tempXhat2 = zeros(size(X));
            tempXhat2(abs(tempxhat)>1e-6) = tempxhat(abs(tempxhat)>1e-6);
        
            estHist.testxvar(it) = trace((X-tempxhat)*(X-tempxhat)')/N/L;
            temppp = estHist.testxvar(it);
            estHist.testAvar(it) = trace((A-tempAhat)*(A-tempAhat)')/M/N;
            tempaa = estHist.testAvar(it);
...
save('C:\Users\DELL\Desktop\Paper use code\_result\Big_AMP_Aerror_hist','Aerror_hist');
save('C:\Users\DELL\Desktop\Paper use code\_result\Big_AMP_Xerror_hist','Xerror_hist');
save('C:\Users\DELL\Desktop\Paper use code\_result\Big_AMP_Avar_hist','Avar_hist');
save('C:\Users\DELL\Desktop\Paper use code\_result\Big_AMP_Xvar_hist','Xvar_hist');
% save('C:\Users\DELL\Desktop\Paper use code\_result\Big_AMP_it_hist','it_hist');
%save('C:\Users\DELL\Desktop\Paper use code\_result\Big_AMP_Zerror_hist','Zerror_hist');
