...
SparseK2 = [0.4];
SimLen = 1;%
Nrrange = [200];%ceil(5.*Nrange.*log(Nrange));% [100]; % receive antenna
...
for iN = 1:length(Nrrange)
    for iK2 = 1:length(SparseK2)
        optIn.N = Ntrange(1); %
        optIn.M = 20;%optIn.N; % coherence time
        optIn.K = round(SparseK(1)*optIn.N);
...
%                 optIn.ScalarMean = ScalarMean(iS);
                ...
                results = DLCommun_LMMSE_ref_SPCA(optIn); % subspace estimation
                ...
                errorA_hist(iS,iK2) = Aerror;
                errorX_hist(iS,iK2) = Xerror;
            end
        end
        
    end
end
save('D:/AMP_Neural/_result/SPCA_KL_errA_change_SNR.mat','errorA_hist');
save('D:/AMP_Neural/_result/SPCA_KL_errX_change_SNR.mat','errorX_hist');
