classdef SparseEstim < EstimIn
    ...
    % Then SparseEstim is the estimator for a new random variable:
    %   X = X1 with prob p1
    %     = x0 with prob 1-p1
    % where x0 is a constant (default = 0)
    
    % modified to fit CAwgn Turbo
    properties
    ...
    methods
        % Constructor
        function obj = SparseEstim(estim1, p1, x0, varargin)
            ...
            if obj.autoTune && ~obj.disableTune
              ...
              if (obj.counter>0), % don't tune yet
                ...
                switch obj.tuneDim
                    ...
                    otherwise 
                        error('Invalid tuning dimension in SparseEstim');
                end
                if obj.weightFlag
...
%               - abs(xhat).^2;
        ...
        % Compute log probability ratio log(P(x~=0)/P(x=0))
        % Be very careful that p0 should be used
%         function logRatio = computeLogRatio(obj, rhat, rvar)
%             exp1 = exp(-abs(rhat).^2 ./ (rvar + obj.estim1.var0) );
%             exp0 = exp(-abs(rhat).^2 ./ rvar );
%             logRatio = log(obj.p0 .* rvar .* exp1 ./ (obj.p0 .* rvar .* exp1 + (rvar + obj.estim1.var0) .* (1 - obj.p0) .* exp0));
%         end
        function logRatio = computeLogRatio(obj, rhat, rvar)
            prob1 = obj.p0 .* (obj.estim1.prob1 .* exp(-abs(rhat - 1).^2 ./ rvar) + ...
                obj.estim1.probm1 .* exp(-abs(rhat + 1).^2 ./ rvar) + ...
                obj.estim1.probi .* exp(-abs(rhat - 1i).^2 ./ rvar) + ...
                obj.estim1.probmi .* exp(-abs(rhat + 1i).^2 ./ rvar));
            prob0 = (1 - obj.p0) .* exp(-abs(rhat).^2 ./ rvar );
            logRatio = log(prob1 ./ prob0);
        end
        
        % Compute probability of X~=0
        % Be very careful that p0 should be used
        function X1 = computeX1(obj, rhat, rvar)
            exp1 = exp(-abs(rhat).^2 ./ (rvar + obj.estim1.var0) );
            exp0 = exp(-abs(rhat).^2 ./ rvar );
            X1 = obj.p0 .* rvar .* exp1 ./ (obj.p0 .* rvar .* exp1 + (rvar + obj.estim1.var0) .* (1 - obj.p0) .* exp0);
        end
