...
%Test case
 ...
 Lrange = [60:10:280];
 SNRrange = [50];
 Sparsityrange = [0.05:0.04:0.97];
 result = zeros(length(Sparsityrange),length(Lrange));
 for iL = 1:length(Lrange)
     for iSNR = 1:length(SNRrange)
         for iSp = 1:length(Sparsityrange)
            %Problem dimensions
            optIn.M = 20; %size of signal
            optIn.N = 40; %size of dictionary
            optIn.L = Lrange(iL); %From Spielman et al.


            %Specify dictionary usage
            optIn.K = round(optIn.N*Sparsityrange(iSp));

            %Specify maximum allowed trials
            optIn.maxTrials = 1;

            %SNR
            optIn.SNR = SNRrange(iSNR);
            ...
            result(iSp,iL) = estHistEM.errZ(end);
         end
     end
 end
 save('C:\Users\DELL\Desktop\Paper use code\_result\DL_overcomplete_KL_Phase_Transition','result');
